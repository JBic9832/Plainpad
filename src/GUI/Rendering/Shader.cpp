...
{
	unsigned int transformLoc = glGetUniformLocation(mRendererId, uniformName.c_str());
...
{
	unsigned int vecLoc = glGetUniformLocation(mRendererId, uniformName.c_str());
...
{
	unsigned int loc = glGetUniformLocation(mRendererId, uniformName.c_str());
...
void Shader::createShader(const std::string vertexPath, const std::string fragmentPath)
{
	...
	checkShaderProgramLinkStatus(mRendererId);
